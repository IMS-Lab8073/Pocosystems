...
class LRFViewer(OpenRTM_aist.DataFlowComponentBase):
    ...
    def onInitialize(self):
        ...
        self._d_rangeData = RTC.RangeData(RTC.Time(0,0), [], RTC.RangerGeometry(_geometry3D, []), _rangerConfig)
...
class LRFrange(ScaledObject):
    ...
    def create_ctrl(self, frame):
        ...
        for w in [text, line, fill, thresh, thresh_scale, tfilter, tfilter_scale, sfilter, sfilter_scale]:
...
def main():
    m = TkLRFViewer(Tk())
    m.master.title("Human_Tracking Viewer")
    ...
    mgr.runManager(True)
    lrf_rtc = mgr.createComponent("Human_Tracking")
